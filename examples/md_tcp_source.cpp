...
private:
    ...
    void accept_all() {
        while (la_socket.can_accept()) {  // && clients.size() < max_incoming_connections &&
            ...
            auto it = --clients.end();
            clients.back().reset( new crab::TCPSocket([this, it]() { on_client_handler(it); }, [this, it]() { on_client_disconnected(it); }));
            std::string addr;
...
int main(int argc, char *argv[]) {
	std::cout << "This server generates stream of 'financial messages' and makes it available via TCP (transmissions, low latency) and HTTP (retransmissions)" << std::endl;
	crab::RunLoop runloop;
