...
public:
    ...
    : settings(settings), message_handler(std::move(message_handler))
    , incoming_socket([&](){ on_incoming_socket_data(); }, [&](){ on_incoming_socket_closed(); })
    , incoming_socket_buffer(4096)
    , udp_a(settings.md_gate_udp_a_address, settings.md_gate_udp_a_port, [&](){}) // We just skip packets if buffer is full in UDP line A
...
int main(int argc, char *argv[]) {
	std::cout << "This gate connects to running instance of md_tcp_source, and broadcasts data via UDP, with support of retransmission requests via HTTP" << std::endl;
	crab::RunLoop runloop;
