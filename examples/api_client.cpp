...
public:
	explicit ApiClientApp(const crab::Address &address, int64_t & external_duration, size_t & external_count, int64_t & external_max_latency)
	    : external_duration(external_duration), external_count(external_count), external_max_latency(external_max_latency), address(address)
...
int main(int argc, char *argv[]) {
	...
	crab::Timer t0([&]{
		...
		std::cout << "msg sent/avg latency/max latency=" << external_count << "/" << lat << " microsec/" << external_max_latency << " microsec" << std::endl;
		external_duration = 0;
		external_count = 0;
