...
void check_digest(const std::string &src, const std::string &digest) {
	...
	invariant(crab::to_hex(result, sizeof(result)) == digest, "");

	for (size_t i = 0; i != src.size(); ++i) {
		uint8_t result2[crab::md5::hash_size]{};
		T{}.add(src.data(), i).add(src.data() + i, src.size() - i).finalize(result2);
		invariant(crab::to_hex(result2, sizeof(result2)) == digest, "");
	}
}
...
int main() {
	...
	check_digest<crab::sha1>("abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu", "a49b2446a02c645bf419f995b67091253a04a259");
