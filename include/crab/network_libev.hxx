...
CRAB_INLINE void Timer::once(steady_clock::duration delay) {
    throw std::logic_error("TODO - implement");  // to convert std::chrono to double
}
...
CRAB_INLINE Watcher::Watcher(Handler &&a_handler) : loop(RunLoop::current()), a_handler(std::move(a_handler)), impl(RunLoop::current()->get_impl()) {
...
CRAB_INLINE bool Idle::is_active() {
...
CRAB_INLINE steady_clock::time_point RunLoop::now() {
    return steady_clock::time_point{}; // TODO convert impl.now();
}
