...
CRAB_INLINE void encode(char *out, size_t out_len, const uint8_t *in, size_t in_len) {
...
CRAB_INLINE std::string encode(const uint8_t *in, size_t in_len) {
	std::string result(encoded_size(in_len), '?');
	encode(&result[0], result.size(), in, in_len);
	return result;
}

CRAB_INLINE bool decode(std::vector<uint8_t> *result, const std::string &data) {
