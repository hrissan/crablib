...
CRAB_INLINE RequestParser::State RequestParser::consume(char input) {
	...
	case URI_PERCENT2: {
		...
		req.path.push_back(static_cast<char>(static_cast<uint8_t>(percent1_hex_digit * 16 + digit2)));
		return URI;
	}
	...
	case FIRST_HEADER_LINE_START:  // Cannot contain LWS
		...
		header.name.push_back(static_cast<char>(std::tolower(input)));
		return HEADER_NAME;
...
CRAB_INLINE void RequestParser::process_ready_header() {
	...
	if (header.name == string_view{"content-length"}) {
	...
	if (header.name == string_view{"transfer-encoding"}) {
		tolower(header.value);
		if (header.value == string_view{"chunked"}) {
